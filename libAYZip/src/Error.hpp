...
public:
    ...
    {
        return "com.rileytestut.AltSigner.Error";
    }

    const char *what() const noexcept override
    {
        try {
            _whatCache = this->localizedDescription();
            if (_whatCache.empty()) {
                _whatCache = "Unknown error.";
            }
    ...
    friend std::ostream &operator<<(std::ostream &os, const Error &error)
    {
        ...
        os << "Error: " << error.localizedDescription() << " (" << error.code() << ")";
        return os;
    }

private:
    int _code;
    mutable std::string _whatCache;
};
